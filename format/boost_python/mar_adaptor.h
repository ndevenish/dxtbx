...
namespace dxtbx { namespace format { namespace boost_python {
  ...
  public:
    ...
    void read_header();
    inline virtual bool header_read_specific(img_handle, FILE*, int*) {
      return false;
    }
    // can't make it abstract or Boost python barfs
    void read_data();
    inline virtual bool data_read_specific(img_handle, FILE*, int*) {
      return false;
    }
    ...
    inline int size1() {
      read_header();
      return img_org_data[0];
    }
    inline int size2() {
      read_header();
      return img_org_data[1];
    }
    af::flex_int rawdata();
  };
  ...
  public:
    ...
    inline double overload() {
      read_header();
      return 249862;  // determined from a single image.
    }
    inline double osc_range() {
      read_header();
      return get_number("OSCILLATION RANGE");
    }
    inline double osc_start() {
      read_header();
      return get_number("PHI");
    }
    inline double twotheta() {
      read_header();  // XXX Unknown whether 0.001 factor is needed
      return get_number("TWOTHETA");
    }
    inline double exposure_time() {
      read_header();
      return get_number("EXPOSURE TIME");
    }
    inline std::string test(std::string item) {
      read_header();
