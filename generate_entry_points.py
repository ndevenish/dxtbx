...
if __name__ == "__main__":
    ...
    lines = [
        f"# Automatically generated by generate_entry_points.py on {datetime.datetime.now(datetime.timezone.utc).isoformat()}",
        "# -- do not alter --",
        "",
    ]
    ...
    for file in package_path.joinpath("command_line").glob("*.py"):
        lines.extend(
            x.strip() for x in get_entry_point(file, "dxtbx", "dxtbx.command_line")
        )
    lines.extend(["", "[dxtbx.format]"])
    lines.extend(sorted(enumerate_format_classes(package_path / "format")))
    Path("entry_points.ini").write_text("\n".join(lines) + "\n")
    print("Written updated entry_points to entry_points.ini")
